...
switch(state)
{
	...
	case NPCSTATES.FINDINGCOVER:
	{
		...
		for(var i = 0; i < instance_number(oCoverParent); i++)
		{
			...
			{
				if(oGame.map_grid[# tile_list[i][0], tile_list[i][1]] != noone) continue
				for(var k = 0; k < array_length(oGame.team_one); k++)
				{
					...
					var tmp_dist1 = point_distance(tile_list[i][0], tile_list[i][1], tmp_x2, tmp_y2)
					var tmp_dist2 = point_distance(tmp_x3, tmp_y3, tmp_x2, tmp_y2)
					if(tmp_dist1 > tmp_dist2) dest = [TileToScreenX(tile_list[i][0], tile_list[i][1]), TileToScreenY(tile_list[i][0], tile_list[i][1])]
					if(dest != undefined) break
					//Back up incase no suitable destination can be found, finds the closest
					if(tmp_dest == undefined) tmp_dest = [TileToScreenX(tile_list[i][0], tile_list[i][1]), TileToScreenY(tile_list[i][0], tile_list[i][1])]
					else if(point_distance(npc.x, npc.y, TileToScreenX(tile_list[i][0], tile_list[i][1]), TileToScreenY(tile_list[i][0], tile_list[i][1])) < point_direction(npc.x, npc.y, tmp_dest[0], tmp_dest[1])) tmp_dest = [tmp_cov.x, tmp_cov.y]
				}
			    if(dest != undefined) break
			}
			if(dest != undefined) break
	...
	case NPCSTATES.MOVING:
	{
		...
		npc.movement_grid[# ScreenToTileX(dest[0], dest[1]), ScreenToTileY(dest[0], dest[1])] = 2
